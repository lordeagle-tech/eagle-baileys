...
const normalizeQuickReplyButtons = buttons => {
  if (!Array.isArray(buttons) || buttons.length < 1 || buttons.length > 3) {
    throw new Boom('buttons must contain between 1 and 3 quick replies', {
      statusCode: 400
    });
  }
  const ids = new Set();
  return buttons.map((button, index) => {
    const id = button?.id ?? button?.buttonId;
    const displayText = button?.displayText ?? button?.buttonText?.displayText;
    if (typeof id !== 'string' || !id.trim() || id.length > 256) {
      throw new Boom(`buttons[${index}].id must be a non-empty string no longer than 256 characters`, {
        statusCode: 400
      });
    }
    if (typeof displayText !== 'string' || !displayText.trim() || displayText.length > 20) {
      throw new Boom(`buttons[${index}].displayText must be a non-empty string no longer than 20 characters`, {
        statusCode: 400
      });
    }
    const normalizedId = id.trim();
    if (ids.has(normalizedId)) {
      throw new Boom(`duplicate button id: ${normalizedId}`, {
        statusCode: 400
      });
    }
...
export const generateWAMessageContent = async (message, options) => {
  ...
  if (hasNonNullishProperty(message, 'buttons')) {
    ...
    if (typeof bodyText !== 'string' || !bodyText.trim()) {
      throw new Boom('button messages require non-empty text or caption', {
        statusCode: 400
      });
    }
    const buttons = normalizeQuickReplyButtons(message.buttons);
    let header;
    if (message.image) {
      ...
      header = {
        hasMediaAttachment: true,
        videoMessage: mediaContent.videoMessage
      };
      ...
      header = {
        hasMediaAttachment: true,
        documentMessage: mediaContent.documentMessage
      };
    } else if (message.location) {
      header = {
        hasMediaAttachment: true,
        locationMessage: WAProto.Message.LocationMessage.create(message.location)
      };
    } else if (message.product) {
      const productContent = await prepareWAMessageMedia({
      ...
      header = {
        ...
        productMessage: WAProto.Message.ProductMessage.create({
          ...message.product,
          productImage: productContent.imageMessage
        })
      };
    ...
    m.groupInviteMessage.groupJid = message.groupInvite.groupJid ?? message.groupInvite.jid;
    m.groupInviteMessage.groupName = message.groupInvite.groupName ?? message.groupInvite.subject;
    if (message.groupInvite.jpegThumbnail) {
      m.groupInviteMessage.jpegThumbnail = message.groupInvite.jpegThumbnail;
    } else if (options.getProfilePicUrl && m.groupInviteMessage.groupJid) {
      try {
        const pfpUrl = await options.getProfilePicUrl(m.groupInviteMessage.groupJid, 'preview');
        if (pfpUrl) {
        ...
        // ignore profile pic fetch errors
      }
    }
  ...
  const topLevelContext = message.contextInfo || {};
  if (message.isForwarded) topLevelContext.isForwarded = message.isForwarded;
  if (message.forwardingScore) topLevelContext.forwardingScore = message.forwardingScore;
  if (message.forwardedNewsletterMessageInfo) topLevelContext.forwardedNewsletterMessageInfo = message.forwardedNewsletterMessageInfo;
  if (message.externalAdReply) topLevelContext.externalAdReply = message.externalAdReply;

  if (Object.keys(topLevelContext).length > 0) {
    const messageType = Object.keys(m)[0];
    const key = m[messageType];
...
export const normalizeMessageContent = content => {
  ...
  function getFutureProofMessage(message) {
    return message?.ephemeralMessage || message?.viewOnceMessage || message?.documentWithCaptionMessage || message?.viewOnceMessageV2 || message?.viewOnceMessageV2Extension || message?.editedMessage || message?.associatedChildMessage || message?.groupStatusMessage || message?.groupStatusMessageV2 || message?.groupMentionMessage || message?.groupMentionMessageV2 || message?.groupInviteMessage || message?.groupInviteMessageV2 || message?.groupMessageV2;
  }
};
export const getButtonReplyInfo = message => {
  const content = normalizeMessageContent(message?.message || message);
  if (!content) {
    return undefined;
  }
  const makeReply = (id, displayText, type) => {
  ...
  if (legacy) {
    return makeReply(legacy.selectedButtonId, legacy.selectedDisplayText, 'legacy');
  }
  const template = content.templateButtonReplyMessage;
  if (template) {
    return makeReply(template.selectedId, template.selectedDisplayText, 'template');
  }
  ...
  try {
    ...
    return makeReply(params.id ?? params.selected_id ?? params.button_id ?? params.row_id, params.display_text ?? params.title ?? params.text ?? interactive.body?.text, 'interactive');
  } catch {
    return undefined;
  }
};
