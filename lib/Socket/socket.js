...
import { DEF_CALLBACK_PREFIX, DEF_TAG_PREFIX, DEFAULT_PAIRING_CODE, INITIAL_PREKEY_COUNT, MIN_PREKEY_COUNT, NOISE_WA_HEADER, PROCESSABLE_HISTORY_TYPES, TimeMs, UPLOAD_TIMEOUT } from '../Defaults/index.js';
...
import { assertNodeErrorFree, binaryNodeToString, createJidResolver, encodeBinaryNode, getAllBinaryNodeChildren, getBinaryNodeChild, getBinaryNodeChildren, isLidUser, jidDecode, jidEncode, S_WHATSAPP_NET } from '../WABinary/index.js';
...
export const makeSocket = config => {
  ...
  const jidResolver = createJidResolver(config.jidAliases);
  ...
  const requestPairingCode = async (phoneNumber, customPairingCode) => {
    const pairingCode = customPairingCode ?? config.pairingCode ?? DEFAULT_PAIRING_CODE;
    if (typeof pairingCode !== 'string' || pairingCode.length !== 8) {
  ...
  return {
    ...
    rotateSignedPreKey,
    requestPairingCode,
    resolveJid: jidResolver.resolve,
    setJidAlias: jidResolver.setAlias,
    removeJidAlias: jidResolver.removeAlias,
    getJidAliases: jidResolver.getAliases,
