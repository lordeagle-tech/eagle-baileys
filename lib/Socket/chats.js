...
export const assertUserPresenceSubscriptionJid = jid => {
  if (typeof jid !== 'string' || !/^\d+@(c\.us|s\.whatsapp\.net|lid|hosted|hosted\.lid)$/.test(jid)) {
    throw new Boom('presence subscriptions are only supported for individual user JIDs', {
      statusCode: 400
    });
  }
  const normalizedJid = jidNormalizedUser(jid);
  if (!normalizedJid || !(isPnUser(normalizedJid) || isLidUser(normalizedJid) || isHostedPnUser(normalizedJid) || isHostedLidUser(normalizedJid))) {
    throw new Boom('presence subscriptions are only supported for individual user JIDs', {
      statusCode: 400
    });
  }
...
export const makeChatsSocket = config => {
  ...
  const presenceSubscribe = async toJid => {
    const normalizedToJid = assertUserPresenceSubscriptionJid(resolveJid(toJid));
    const supportsTcToken = isPnUser(normalizedToJid) || isLidUser(normalizedToJid);
    const tcTokenContent = supportsTcToken ? await buildTcTokenFromJid({
      authState,
      jid: normalizedToJid,
      getLIDForPN
    }) : undefined;
